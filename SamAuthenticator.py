...
idpentryurl = 'https://fs.natura.net/adfs/ls/IdpInitiatedSignOn.aspx'

#https://fs.natura.net/adfs/ls/IdpInitiatedSignOn.aspx
...
payload = {'RelyingParty': '7a560aab-218e-ea11-80f5-005056a5355b', 'idp_OtherRpRadioButton': 'SignInOtherSite', 'SignInOtherSite': 'SignInOtherSite', 'SignInSubmit': 'Sign in', 'SingleSignOut': 'SingleSignOut'}

response = session.post(idpentryurl, data=payload, verify=sslverification, timeout=10)

formsoup = BeautifulSoup(response.text)
...
#s3conn = boto3.client('s3', aws_access_key_id=token['Credentials']['AccessKeyId'], aws_secret_access_key=token['Credentials']['SecretAccessKey'], aws_session_token=token['Credentials']['SessionToken'])
